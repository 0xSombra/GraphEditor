...
namespace GraphEditor.Renderer
{
    ...
    /// See https://github.com/mellinoe/ImGui.NET/blob/master/src/ImGui.NET.SampleProgram.XNA/ImGuiRenderer.cs
    /// </summary>
    public class ImGuiRenderer
    {
        ...
        // Input
        private int _scrollWheelValueH;
        private int _scrollWheelValueV;

        private List<int> _keys = new List<int>();

        unsafe void LoadFonts()
        {
            static ImFontConfig NewImFontConfig()
            {
                unchecked
                {
                    return new ImFontConfig()
                    {
                        // Copied from ImFontConfig::ImFontConfig() in imgui

                        //FontData = NULL,
                        FontDataSize = 0,
                        FontDataOwnedByAtlas = 1, // bool
                        FontNo = 0,
                        SizePixels = 0.0f,
                        OversampleH = 3,
                        OversampleV = 1,
                        PixelSnapH = 0, // bool
                        GlyphExtraSpacing = new System.Numerics.Vector2(0.0f, 0.0f),
                        GlyphOffset = new System.Numerics.Vector2(0.0f, 0.0f),
                        //GlyphRanges = NULL,
                        GlyphMinAdvanceX = 0.0f,
                        GlyphMaxAdvanceX = float.MaxValue, // FLT_MAX
                        MergeMode = 0, // bool
                        FontBuilderFlags = 0x00, // RasterizerFlags
                        RasterizerMultiply = 1.0f,
                        EllipsisChar = (ushort)-1,
            ...
            var io = ImGui.GetIO();
            var fontConfig = NewImFontConfig();
            fontConfig.OversampleH = 4;
            fontConfig.OversampleV = 4;
            fontConfig.PixelSnapH = 0; // bool

            io.Fonts.Clear();
            var fontPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Fonts), "arial.ttf");
            io.Fonts.AddFontFromFileTTF(fontPath, 14.0f, new ImFontConfigPtr(&fontConfig));
            io.Fonts.Build();
        }
        ...
        protected virtual void UpdateInput()
        {
            ...
            var scrollDeltaH = mouse.HorizontalScrollWheelValue - _scrollWheelValueH;
            io.MouseWheelH = -(scrollDeltaH > 0 ? 1 : scrollDeltaH < 0 ? -1 : 0);
            _scrollWheelValueH = mouse.HorizontalScrollWheelValue;

            var scrollDeltaV = mouse.ScrollWheelValue - _scrollWheelValueV;
            io.MouseWheel = scrollDeltaV > 0 ? 1 : scrollDeltaV < 0 ? -1 : 0;
            _scrollWheelValueV = mouse.ScrollWheelValue;
        }
